...
def detect_age_gend(image):
    results = model_age_gend(image)
    return results
...
def pedestrian_crop(pedestrian_results, original_image):
    ...
    Returns:
        ...
        detected pedestrian, and values are another dictionary containing the
        'bbox', 'apparel_results', and 'age_gender_results'.
    """
    all_detections = {}
    pedestrian_id = 0
    ...
    for result in pedestrian_results:
        ...
        for box in boxes:
            ...
            if cropped_pedestrian.size > 0:
                ...
                age_gender_results = detect_age_gend(cropped_pedestrian)
...
def log_detections_to_csv(detections, csv_filepath, frame_id=0):
    ...
    with open(csv_filepath, 'a', newline='') as csvfile:
        ...
        for ped_id, data in detections.items():
            ped_bbox = data['bbox']
            ped_x1, ped_y1, _, _ = ped_bbox

            # Helper function to process results (apparel or age/gender)
            def process_results(results, detection_type):
                for res in results:
                    class_names = res.names
                    for box in res.boxes:
                        label = class_names[int(box.cls)]
                        confidence = float(box.conf)
                        item_bbox = box.xyxy[0].cpu().numpy().astype(int)
                        
                        # Convert item bbox to original image coordinates
                        ix1, iy1, ix2, iy2 = item_bbox
                        writer.writerow([
                            frame_id, ped_id, ped_bbox[0], ped_bbox[1], ped_bbox[2], ped_bbox[3],
                            detection_type, label, f"{confidence:.4f}",
